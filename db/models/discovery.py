...
class InmuebleRaw(Base):
    """Staging area de anuncios inmobiliarios scrapeados"""
...
class DeteccionAnuncio(Base):
    """Detecciones de posibles coincidencias entre anuncios y inmuebles del CORE"""
    __tablename__ = "portals_detecciones"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    inmueble_id: Mapped[int] = mapped_column(Integer, ForeignKey("portals_inmuebles_raw.id"), index=True)

    # El vínculo con el CORE se guarda aquí (esto es lo que el usuario confirma)
    inmueble_core_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("inmuebles.id"), index=True)
